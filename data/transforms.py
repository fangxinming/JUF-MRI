...
from math_lib import ifft2c, fft2c, complex_abs
...
class DataTransform(object):
    ...
    def __init__(self, which_challenge, hr_in):
        """
        Args:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        print(kspace.dtype)  # 检查输入张量的数据类型
        image = ifft2c(kspace)
        ...
        imgfft = fft2c(image)
        if self.hr_in == False:
            imgfft = complex_center_crop(imgfft, (80, 80))
        else:
            imgfft = imgfft* mask
...
def apply_mask(
    ...
    mask = mask_func(shape, seed)
    h,w,c = data.shape
    mask = mask.repeat(h,1,2)

    masked_data = data * mask # the + 0.0 removes the sign of the zeros
...
class ReconstructionTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        # cv2.imwrite('./image.png', to_image((image*std+mean).numpy()))
        ...
        return image, target, mean, std, fname, slice_num, mask, masked_kspace


def build_transforms(mode = 'train', hr_in = 'True'):

    # if mode == 'train':
    #     mask = create_mask_for_mask_type('random', [0.08], [4])
    #     return ReconstructionTransform('singlecoil', mask, use_seed=False)
    # elif mode == 'val':
    #     mask = create_mask_for_mask_type('random', [0.08], [4])
    #     return ReconstructionTransform('singlecoil', mask)
    # else:
    #     return ReconstructionTransform('singlecoil')


    # if args.WORK_TYPE == 'sr':
    if mode == 'train':
        return DataTransform('singlecoil', hr_in)
    elif mode == 'val':
        return DataTransform('singlecoil', hr_in)
    else:
        return DataTransform('singlecoil', hr_in)
