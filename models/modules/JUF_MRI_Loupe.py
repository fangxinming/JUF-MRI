...
class LoupeLayer(nn.Module):
    ...
    def sparsify(self, mask):
        ...
        r = self.sparsity / xbar  # 稀疏度除以均值    要求的稀疏度除以目前的稀疏度
    ...
    def forward(self):
        ...
        probmask = self.squash_mask(self.pmask)  # 初始化并把数据映射到[0.01.0.99]之间
...
class Encoding_Block(torch.nn.Module):
    def __init__(self, c_in, n_feat):
        ...
        self.conv4 = torch.nn.Conv2d(in_channels=n_feat, out_channels=n_feat, kernel_size=3, stride=2, padding=3 // 2) #步幅为2,尺寸减少一半
...
class JUF_MRI_Loupe(nn.Module):
    def __init__(self, opt):
        # 此处的opt为opt_net 为opt['network_G']
        super(JUF_MRI_Loupe, self).__init__()
