...
class DCN_sep(DCNv2):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1,
        ...
        channels_ = self.deformable_groups * 3 * self.kernel_size[0] * self.kernel_size[1]
        #deformable_groups=4  4*3*3*3 108
        self.conv_offset_mask = nn.Conv2d(32, channels_, kernel_size=self.kernel_size,
                                          stride=self.stride, padding=self.padding, bias=True)

        # nn.Conv2d(32,channels_=108,kernel_size=3,stride=1,padding=1,bias=True)
        self.init_offset()
        # 卷积参数初始化为0
    ...
    def forward(self, input, fea):
        ...
        out = self.conv_offset_mask(fea)  #fea (B,32,H,W) out(B,108,H,W)
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        # torch.chunk函数把一个张量分割成若干子张量 分为三个子张量 o1(B,36,H,W) o2(B,36,H,W) mask(B,36,H,W)
        offset = torch.cat((o1, o2), dim=1)
        # offset(B,72,H,W)
        mask = torch.sigmoid(mask)
