...
#
from skimage.metrics import peak_signal_noise_ratio as calculate_psnr # 使用 scikit-image 计算 PSNR
...
def main():
    ...
    dataset_opt['scale'] = 4  # acceleration
    dataset_opt['undersample'] = 'equ'  # 'random' or 'equ' or 'learned' or 'radial' or 'spiral'
    dataset_opt['network'] = 'TitanRNet_freloss15s3'
    # 'MCDudo' 'DuDoNet_Loupe' 'VANet' 'CUNet' 'restormer' 'MDUNet' 'TitanRNet_freloss15s3' 'TitanRNet_freloss15s3_diffusion'
    ...
    model_path = '/home/xinming/MCDudo/experiments/' + name + ('/models/Iter_79800.pth')
    #print(model_path)


    model = JUF_MRI_Loupe.JUF_MRI_Loupe(dataset_opt).cuda()

    #model = JUF_MRI_diffusion.TitanRNet_diffusion(dataset_opt).cuda()
    ...
    # checkpoint['LoupeLayer.pmask'] = torch.randn(240, 240)  # 修改为当前模型的形状
    ...
    with torch.no_grad():
        ...
        for i, val_data in enumerate(tqdm(val_loader)):
            ...
            for batch_idx in range(im1_lr.size(0)):
                ...
                # cur_psnr_im1 = util.calculate_psnr(sr_img_1, im1_gt_batch)
                # avg_psnr_im1 += cur_psnr_im1
                # cur_ssim_im1 = util.calculate_ssim(sr_img_1, im1_gt_batch)
                ...
                if save_result:
                    save_path = '/home/xinming/MCDudo/test/test_1' + name
