...
def compute_soft_edge_tensor(x: torch.Tensor) -> torch.Tensor:
    """
...
def setup_logger(logger_name, root, phase, level=logging.INFO, screen=False, tofile=False):
    #用于设置日记记录器 logger_name用于设置日记的名称，root日志文件的根目录
    #phase用于表示不同的日志阶段 level日志记录的级别 screen是否输出到屏幕 tofile是否输出到文件
    '''set up logger'''
    lg = logging.getLogger(logger_name)
    #创建一个名为‘logger_name’的日志
...
def calculate_psnr(img1, img2):
    ...
    #先计算两个图像的差值,然后对每个像素的差值进行平方,最后取均值
    if mse == 0:
        return float('inf')
...
# def calculate_psnr_fastmri(img1, img2):
#     img1 = img1.astype(np.float64)
#     img2 = img2.astype(np.float64)
